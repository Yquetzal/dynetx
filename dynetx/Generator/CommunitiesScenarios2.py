...
from dynetx.utils.DynComAnalysis import computeNetworkStability


comSteps = dict()

class abstractStructure():
    #this class can be instanciated as a community or an ongoing operation
    ...
    #Return all pairs of nodes inside the structure
    def getInternPairs(self):
        pass
...
class community(abstractStructure):
    ...
    def __init__(self,comScenario,name=None):
        ...
        id = comScenario.getNewID(prefix="COM")
    ...
    def addNodes(self, nodes): #used to initialize a community size
         for n in nodes:
             self._addNode(n)
    ...
    def getNbInternEdges(self):
        ...
        #Return automatically the desired number of edges for this community given its size. Should be based on a parameter alpha

        #return (math.sqrt(len(self._nodes)) + 1) * len(self._nodes) / 2
        #nbPairs = self._nodes  * len(self._nodes-1)/2
        nbNodes = len(self._nodes)
        #return (1+math.pow(nbNodes-2,1/1.3))*nbNodes/2 #1 node certain and among the others we chose with a root function
        alpha=0.75
        #print("nbN ",nbNodes,self.getName())
        return math.ceil((math.pow(nbNodes-1,alpha)*nbNodes)/2)
        #return (self.desiredDensity*len(self.getInternPairs()))
    ...
    def getInternEdges(self,variant="best"):
        # select the pairs with highest score
        sortedPairs = sorted(self.getInternPairs().items(), key=operator.itemgetter(1), reverse=True)
...
class operation(abstractStructure):
    ...
    def initialise(self,comScen):
        ...
        for comName in self.beforeIDS:
            self.before.append(self.comScenario.retrieveLastCommunityWithName(comName))
...
class comScenario():
    ...
    def run(self):
        ...
        while len(self.actions)>0 or len([x for x in self.abstractCommunities.values() if type(x) is operation])>0:
            ...
            readycomIDs = {c.getID() for c in self.abstractCommunities.values()}
...
def theseusBoat(theComTh, wait=3):
    ...
    for i in range(size):
        xCom = comScen.addAction(operation("birth", parameters={"size": 1}), waitFor=currentID,
                                 wait=wait)  ####HERE we need a way to wait for the community. Maybe using wait parameter or a new one.
        comPlus1 = comScen.merge([name, xCom[0].getName()], [name])
        nodeToRemove = initialNodes[i]
        currentID = comPlus1[0].getID()
...
def migration(comFrom, comTo, nbNodes, wait=0):
    ...
    for i in range(nbNodes):
        ...
        comScen.merge([comTo.getName(), switched.getName()], [comTo.getName()], waitFor=switched.getID())
...
def growIterative(com, nodes2Add, wait=1):
    ...
    for i in range(nodes2Add):
        xCom = comScen.addAction(operation("birth", parameters={"size": 1}), waitFor=currentCom.getID(),
                                 wait=wait)  ####HERE we need a way to wait for the community. Maybe using wait parameter or a new one.
        comPlus1 = comScen.merge([currentCom.getName(), xCom[0].getName()], [currentCom.getName()])
        currentCom = comPlus1[0]
    return currentCom